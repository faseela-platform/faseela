  ...
  return (
    <div className="max-w-xl">
      <p className="text-caption mb-2 text-[var(--ink-muted)]">
        الحالة:{" "}
        <span className="font-semibold text-[var(--ink)]">{STATE_LABEL[initial.state]}</span>
      </p>

      <label htmlFor="track-title" className="text-caption mb-1 block text-[var(--ink-muted)]">
        العنوان
      </label>
      <input
        id="track-title"
      ...
      <label htmlFor="track-summary" className="text-caption mb-1 block text-[var(--ink-muted)]">
        الوصف
      </label>
      <textarea
        id="track-summary"
      ...
      <label htmlFor="track-slug" className="text-caption mb-1 block text-[var(--ink-muted)]">
        المُعرّف (لاتيني)
      </label>
      <input
        id="track-slug"
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {initial.state !== "published" ? (
          <button
            type="button"
            disabled={pending}
            onClick={() => run(() => publishTrackAction(trackId))}
            className={stateBtn}
          >
            انشر
          </button>
        ...
        {initial.state !== "draft" ? (
          <button
            type="button"
            disabled={pending}
            onClick={() => run(() => unpublishTrackAction(trackId))}
            className={stateBtn}
          >
            أعِد إلى مسودة
          </button>
        ...
        {initial.state !== "archived" ? (
          <button
            type="button"
            disabled={pending}
            onClick={() => run(() => archiveTrackAction(trackId))}
            className={stateBtn}
          >
            أرشف
          </button>
