...
- Turborepo + pnpm. `apps/web` (Next 16.3 + Better Auth), `packages/db` (deep module),
  `packages/tokens`. Neon PostgreSQL 18.4 — one `public` schema (our tables). Editors are `user` rows
  with a staff `role` (`member`/`editor`/`admin`); Payload was removed — see ADR 0023.
- `apps/web` is **ESM** (`"type":"module"`) — kept for Next 16's ESM config.
...
against live Neon), Better Auth (Editors are `user` rows with a staff `role`), Track pages on real
seeded data, erasure, sign-in page (`/dukhul` — shows "registration not open" while email is unconfigured, by
