...
describe("dark mode does not reuse light brand steps", () => {
  it("inverts brand roles to the 100-200 range", () => {
    ...
    // Match on the selector name only. An earlier version sliced on `[data-theme='dark']` with single
    // quotes and silently matched nothing once Prettier normalised the CSS to double quotes, so the
    // regex below searched the whole file and found the light-mode value.
